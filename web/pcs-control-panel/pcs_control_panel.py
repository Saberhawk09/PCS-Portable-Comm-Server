...
main{max-width:1450px;margin:auto;padding:1.2rem}.hero{display:grid;grid-template-columns:minmax(0,1.55fr) minmax(260px,.65fr);gap:1rem;margin-bottom:1rem}
...
@media(min-width:1300px){.public-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.admin-grid{grid-template-columns:repeat(4,minmax(0,1fr))}}
...
def render_admin_page(data: dict, csrf: str, result: str = "", action_name: str = "", return_code: int | None = None, nonce: str = "") -> bytes:
    ...
    <main><section class="overview"><div><h2>Administrative health overview</h2><p>Authenticated session · refreshed {esc(data.get('generated_at', 'unknown'))}</p></div>{badge(data.get('overall', 'warn'))}</section>
