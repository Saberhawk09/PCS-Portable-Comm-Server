...
PCS_PISTAR_PAIR="${PCS_PISTAR_PAIR:-ask}"
...
PCS_SETUP_STARLINK_TELEMETRY="${PCS_SETUP_STARLINK_TELEMETRY:-no}"
PCS_STARLINK_ENABLED="${PCS_STARLINK_ENABLED:-no}"
PCS_STARLINK_AUTODETECT="${PCS_STARLINK_AUTODETECT:-no}"
PCS_UPLINK_MODE="${PCS_UPLINK_MODE:-}"
PCS_UPLINK_PRIORITY="${PCS_UPLINK_PRIORITY:-}"
PCS_STARLINK_MAC="${PCS_STARLINK_MAC:-}"
PCS_STARLINK_PROFILE="${PCS_STARLINK_PROFILE:-pcs-starlink-uplink}"
...
PCS_APRS_AGENT_OUTBOUND_RETRY_SECONDS="${PCS_APRS_AGENT_OUTBOUND_RETRY_SECONDS:-30,60,120,240}"
PCS_APRS_AGENT_OUTBOUND_MAX_PENDING="${PCS_APRS_AGENT_OUTBOUND_MAX_PENDING:-100}"
PCS_APRS_AGENT_OUTBOUND_RETENTION_SECONDS="${PCS_APRS_AGENT_OUTBOUND_RETENTION_SECONDS:-604800}"
...
write_install_config() {
...

    {
        ...
        case "${PCS_REINSTALL_STATE_DIR}" in
            /run/pcs-reinstall.*) printf "PCS_REINSTALL_STATE_DIR=%q\n" "" ;;
            *) printf "PCS_REINSTALL_STATE_DIR=%q\n" "${PCS_REINSTALL_STATE_DIR}" ;;
        esac
        printf "PCS_REINSTALL_ARCHIVE=%q\n" "${PCS_REINSTALL_ARCHIVE}"
        ...
        printf "PCS_SETUP_STARLINK_TELEMETRY=%q\n" "${PCS_SETUP_STARLINK_TELEMETRY}"
        printf "PCS_STARLINK_ENABLED=%q\n" "${PCS_STARLINK_ENABLED}"
        printf "PCS_STARLINK_AUTODETECT=%q\n" "${PCS_STARLINK_AUTODETECT}"
        printf "PCS_UPLINK_MODE=%q\n" "${PCS_UPLINK_MODE}"
        printf "PCS_UPLINK_PRIORITY=%q\n" "${PCS_UPLINK_PRIORITY}"
        printf "PCS_STARLINK_MAC=%q\n" "${PCS_STARLINK_MAC}"
        printf "PCS_STARLINK_PROFILE=%q\n" "${PCS_STARLINK_PROFILE}"
        ...
        printf "PCS_APRS_AGENT_ENABLED=%q\n" "${PCS_APRS_AGENT_ENABLED}"
        printf "PCS_APRS_AGENT_ICHANNEL=%q\n" "${PCS_APRS_AGENT_ICHANNEL}"
        printf "PCS_APRS_AGENT_RF_ENABLED=%q\n" "${PCS_APRS_AGENT_RF_ENABLED}"
        printf "PCS_APRS_AGENT_RF_CHANNEL=%q\n" "${PCS_APRS_AGENT_RF_CHANNEL}"
        printf "PCS_APRS_AGENT_TOCALL=%q\n" "${PCS_APRS_AGENT_TOCALL}"
        printf "PCS_APRS_AGENT_DEDUPE_TTL_SECONDS=%q\n" "${PCS_APRS_AGENT_DEDUPE_TTL_SECONDS}"
        printf "PCS_APRS_AGENT_SENDER_RATE_PER_MINUTE=%q\n" "${PCS_APRS_AGENT_SENDER_RATE_PER_MINUTE}"
        printf "PCS_APRS_AGENT_GLOBAL_RATE_PER_MINUTE=%q\n" "${PCS_APRS_AGENT_GLOBAL_RATE_PER_MINUTE}"
        printf "PCS_APRS_AGENT_OUTBOUND_RETRY_SECONDS=%q\n" "${PCS_APRS_AGENT_OUTBOUND_RETRY_SECONDS}"
        printf "PCS_APRS_AGENT_OUTBOUND_MAX_PENDING=%q\n" "${PCS_APRS_AGENT_OUTBOUND_MAX_PENDING}"
        printf "PCS_APRS_AGENT_OUTBOUND_RETENTION_SECONDS=%q\n" "${PCS_APRS_AGENT_OUTBOUND_RETENTION_SECONDS}"
...
choose_setup_mode() {
    ...
    echo "  COMMISSIONED - Rebuild this PCS hardware profile, excluding saved secrets and external-device changes."
    echo "  QUIT     - Exit without changing anything."
    echo

    while true; do
        read -r -p "Choose setup input mode [ASK/ALL/DEFAULTS/COMMISSIONED/QUIT]: " answer
        answer="${answer:-ASK}"
        ...
        case "${answer}" in
            ...
            defaults|DEFAULTS|Defaults|default|DEFAULT|Default)
                ...
                return 0
                ;;
            commissioned|COMMISSIONED|Commissioned|pcs|PCS)
                PCS_SETUP_MODE="COMMISSIONED"
                return 0
                ;;
            quit|QUIT|Quit|q|Q)
                echo "Aborted."
                ...
                ;;
            *)
                echo "Please choose ASK, ALL, DEFAULTS, COMMISSIONED, or QUIT."
                ;;
        esac
    done
}
...
collect_install_answers() {
    PCS_REINSTALL_ARCHIVE="$(ask_value "Encrypted PCS private recovery archive (blank for none)" "${PCS_REINSTALL_ARCHIVE}")"
    local recovery_directory_default="${PCS_REINSTALL_STATE_DIR}"
    # An encrypted archive supersedes stale temporary extraction state from an
    # interrupted earlier attempt. The operator can still type a real directory.
    [[ -n "${PCS_REINSTALL_ARCHIVE}" ]] && recovery_directory_default=""
    PCS_REINSTALL_STATE_DIR="$(ask_value "Trusted decrypted recovery directory (blank for a new install)" "${recovery_directory_default}")"
    local usb_default
    local gps_default
    ...
    case "${PCS_SETUP_MODE}" in
        COMMISSIONED)
            ...
            if [[ -z "${PCS_SAMBA_PASSWORD}" ]]; then
                if [[ -n "${PCS_REINSTALL_ARCHIVE}" ]]; then
                    PCS_SAMBA_PASSWORD="$(openssl rand -base64 24)"
                else
                    ask_secret_confirm "New Samba password for ${PCS_SAMBA_WORKGROUP}\\${PCS_SAMBA_USER}" PCS_SAMBA_PASSWORD
                fi
            fi
            ...
            # External appliances are intentionally not changed by unattended rebuilds.
        ...
        ALL)
            ...
            PCS_PISTAR_PAIR="${PCS_SETUP_PISTAR}"
    ...
    export PCS_SETUP_STARLINK_TELEMETRY
    export PCS_STARLINK_ENABLED
    export PCS_STARLINK_AUTODETECT
    export PCS_UPLINK_MODE
    export PCS_UPLINK_PRIORITY
    export PCS_STARLINK_MAC
    export PCS_STARLINK_PROFILE
...
confirm_install_answers() {
    ...
    echo "  Pi-Star pairing:    ${PCS_PISTAR_PAIR}"
    echo "  WireGuard remote:   ${PCS_SETUP_WIREGUARD}"
    echo "  WireGuard profile:  ${PCS_WIREGUARD_PROFILE}"
    echo "  Recovery directory: ${PCS_REINSTALL_STATE_DIR:-none}"
    echo "  Recovery archive:   ${PCS_REINSTALL_ARCHIVE:-none}"
    if [[ -n "${PCS_REINSTALL_STATE_DIR}" ]]; then
        echo "  Recovery restores saved Wi-Fi, VPN clients/keys, and Android TLS/pairing."
    fi
    ...
    echo "  Starlink telemetry: ${PCS_SETUP_STARLINK_TELEMETRY} (enabled: ${PCS_STARLINK_ENABLED})"
    if [[ "${PCS_SETUP_MODE}" == "COMMISSIONED" ]]; then
        echo "  Deferred external:   Pi-Star pairing, Meshtastic radio/broker credentials"
        echo "  Deferred secrets:    WireGuard identity, APRS-IS, saved Wi-Fi/API identities"
        echo "  RF state:            staged; post-wipe hardware validation is still required"
    fi
    echo
...
PCS_REINSTALL_RUNTIME_DIR=""
cleanup_reinstall_runtime() {
    if [[ -n "${PCS_REINSTALL_RUNTIME_DIR}" ]]; then
        case "${PCS_REINSTALL_RUNTIME_DIR}" in
            /run/pcs-reinstall.*)
                sudo test -d "${PCS_REINSTALL_RUNTIME_DIR}" \
                    && sudo rm -rf -- "${PCS_REINSTALL_RUNTIME_DIR}"
                ;;
            *)
                echo "WARNING: refusing to remove unexpected recovery runtime path: ${PCS_REINSTALL_RUNTIME_DIR}" >&2
                ;;
        esac
    fi
}
trap cleanup_reinstall_runtime EXIT
if [[ -n "${PCS_REINSTALL_ARCHIVE}" ]]; then
    [[ -z "${PCS_REINSTALL_STATE_DIR}" ]] \
        || { echo "ERROR: Choose either an encrypted recovery archive or a decrypted recovery directory, not both." >&2; exit 2; }
    PCS_REINSTALL_RUNTIME_DIR="$(sudo mktemp -d /run/pcs-reinstall.XXXXXX)"
    sudo chmod 0700 "${PCS_REINSTALL_RUNTIME_DIR}"
    ./scripts/pcs-reinstall-state.sh --extract "${PCS_REINSTALL_ARCHIVE}" "${PCS_REINSTALL_RUNTIME_DIR}"
...
ensure_executable "scripts/setup-uplink-manager.sh"
...
ensure_executable "scripts/pcs_aprs_agent.py"
ensure_executable "scripts/setup-pcs-aprs-agent.sh"
ensure_executable "scripts/setup-graywolf-aprs.sh"
ensure_executable "scripts/setup-meshtastic-bluetooth.sh"
ensure_executable "scripts/setup-starlink-telemetry.sh"
...
if [[ "${PCS_SETUP_PISTAR}" == "yes" && "${PCS_PISTAR_PAIR}" != "no" ]]; then
    ...
    echo
    echo "Pi-Star coordinated shutdown pairing is disabled; monitoring policy is ${PCS_SETUP_PISTAR}."
fi
...
if [[ "${PCS_SETUP_WIREGUARD}" == "yes" ]]; then
    ...
    if [[ "${PCS_WIREGUARD_RESTORED}" != "yes" ]] && ! ./scripts/setup-wireguard-management.sh --validate-profile "${WIREGUARD_PROFILE_PATH}"; then
        ...
        if ! PCS_WIREGUARD_PREPARE_CONFIRM=yes ./scripts/setup-wireguard-management.sh --prepare; then
            ...
            if [[ "${PCS_WIREGUARD_RESTORED}" != "yes" ]] && ! PCS_WIREGUARD_IMPORT_REPLACE_CONFIRM=yes ./scripts/setup-wireguard-management.sh --import-profile "${WIREGUARD_PROFILE_PATH}"; then
                WIREGUARD_SETUP_OK="no"
...
if [[ "${PCS_SETUP_STARLINK_TELEMETRY}" == "yes" ]] || sudo test -f /etc/pcs/starlink.json; then
    run_optional_step "Install or update Starlink telemetry" "sudo --preserve-env=PCS_STARLINK_ENABLED bash ./scripts/setup-starlink-telemetry.sh --install"
fi
...
if [[ -n "${PCS_REINSTALL_STATE_DIR}" ]]; then
    ...
    if sudo test -s /etc/pcs/pistar-shutdown/id_ed25519 \
        && sudo test -s /etc/pcs/pistar-shutdown/known_hosts; then
        PCS_PISTAR_PAIR="yes"
        export PCS_PISTAR_PAIR
    fi
    PCS_REINSTALL_STATE_DIR=""
    PCS_REINSTALL_ARCHIVE=""
    export PCS_REINSTALL_STATE_DIR PCS_REINSTALL_ARCHIVE
fi
