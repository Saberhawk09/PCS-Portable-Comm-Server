...
APRS_AUDIO_REFRESH_SERVICE_SRC="${REPO_DIR}/systemd/pcs-aprs-audio-refresh.service"
...
EOF
    ...
    sudo install -o root -g root -m 0644 "${APRS_AUDIO_REFRESH_SERVICE_SRC}" "${APRS_AUDIO_REFRESH_SERVICE_DST}"
    sudo install -d -o root -g root -m 0755 "$(dirname "${APRS_AUDIO_UDEV_RULE_DST}")"
    sudo install -o root -g root -m 0644 "${APRS_AUDIO_UDEV_RULE_SRC}" "${APRS_AUDIO_UDEV_RULE_DST}"
    sudo install -o root -g direwolf -m 0640 "${audio_env}" "${APRS_CONFIG_DIR}/audio.conf"
